...
public class FirstPersonController : MonoBehaviour
{
    ...
    public float yaw = 0.0f; // Rotation around the y axis
    public float pitch = 0.0f; // Rotation around the x axis
    ...
    private void FixedUpdate()
    {
        ...
        if(cameraCanMove)
        {
            yaw = transform.localEulerAngles.y + ImplementedInput.GetMouseX() * mouseSensitivity;

            if (!invertCamera)
            {
                pitch -= mouseSensitivity * ImplementedInput.GetMouseY();
            }
            else
            {
                // Inverted Y
